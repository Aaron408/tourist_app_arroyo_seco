...
import { gastronomyService, Ingredient } from "@/services/gastronomyService";
...
export default function IngredientDetailScreen() {
  ...
  useEffect(() => {
    const fetchIngredient = async () => {
      if (!params.id) {
        setError('ID de ingrediente no proporcionado');
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        const data = await gastronomyService.getIngredientById(ingredientId, language);
        setIngredient(data);
      } catch (err) {
  ...
  // Get translated data
  const language = getLanguageCode(currentLanguage);
  const translation = ingredient ? gastronomyService.getTranslation(ingredient.translations, language) : null;
  const season = ingredient ? gastronomyService.getHarvestSeason(
    ingredient.harvest_start_month,
    ingredient.harvest_end_month
  ) : '';
  const isInSeason = ingredient ? gastronomyService.isInSeason(
    ingredient.harvest_start_month,
      ...
      <View style={styles.container}>
        ...
        >
          <View style={styles.loadingContainer}>
            ...
            <Text style={styles.loadingText}>Cargando ingrediente...</Text>
          </View>
        </LinearGradient>
      </View>
    );
  ...
  // Error state
  if (error || !ingredient || !translation) {
    return (
      <View style={styles.container}>
        <StatusBar style="light" />
        <LinearGradient
        ...
        >
          ...
          <View style={styles.errorContainer}>
            ...
            <Text style={styles.errorText}>
              {error || 'No se pudo cargar el ingrediente'}
            </Text>
            <TouchableOpacity
      ...
      >
        ...
        >
          ...
          <View style={styles.heroSection}>
            ...
            {/* Title */}
            <Text style={styles.ingredientTitle}>{translation.name}</Text>
            ...
            <View style={[styles.seasonBadge, isInSeason && styles.seasonBadgeActive]}>
              <Ionicons
                name="leaf"
                size={16}
                color={isInSeason ? colors.green500 : colors.gray400}
              />
          ...
          {ingredient.unit && (
            <View style={styles.section}>
              <View style={styles.sectionHeader}>
                <Text style={styles.sectionTitle}>⚖️ Unidad de Medida</Text>
              </View>
