...
import { gastronomyService, Technique } from "@/services/gastronomyService";
...
export default function TechniqueDetailScreen() {
  ...
  useEffect(() => {
    const fetchTechnique = async () => {
      if (!params.id) {
        setError('ID de técnica no proporcionado');
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        const techniqueId = parseInt(params.id, 10);
        const data = await gastronomyService.getTechniqueById(techniqueId, language);
        setTechnique(data);
      } catch (err) {
        console.error('Error fetching technique:', err);
        setError('No se pudo cargar la técnica');
      } finally {
        setLoading(false);
      }
    };
  ...
  // Get translated data
  const language = getLanguageCode(currentLanguage);
  const translation = technique ? gastronomyService.getTranslation(technique.translations, language) : null;
      ...
      <View style={styles.container}>
        ...
        >
          <View style={styles.loadingContainer}>
            ...
            <Text style={styles.loadingText}>Cargando técnica...</Text>
          </View>
        </LinearGradient>
      </View>
    );
  ...
  // Error state
  if (error || !technique || !translation) {
    return (
      <View style={styles.container}>
        <StatusBar style="light" />
        <LinearGradient
        ...
        >
          ...
          <View style={styles.errorContainer}>
            ...
            <Text style={styles.errorText}>
              {error || 'No se pudo cargar la técnica'}
            </Text>
            <TouchableOpacity
      ...
      >
        ...
        >
          ...
          <View style={styles.heroSection}>
            ...
            {/* Title */}
            <Text style={styles.techniqueTitle}>{translation.name}</Text>
            <Text style={styles.techniqueBadge}>📚 Técnica Culinaria</Text>
          </View>
